...
for (const [width,height] of [[568,320],[844,390],[1280,590]]) {
  test(`我的页面横屏 ${width}：完整显示、整行按钮可点`, async ({page,context,browserName})=>{
    ...
    for(const title of ['更换头像','修改昵称','退出登录','声音设置','账号安全','帮助与反馈','用户协议与隐私','战队与会员','开桌权限']) {
...
test('普通会员不显示管理入口，长昵称和账号不挤出名片',async({page,context})=>{
  ...
  await expect(page.getByRole('button',{name:'开桌权限',exact:true})).toHaveCount(0);
...
test('修改昵称、声音持久化、密码入口、反馈、管理和退出均可操作',async({page,context})=>{
  ...
  for(const title of ['战队与会员','开桌权限']){await page.getByRole('button',{name:title,exact:true}).click();await expect(page.getByRole('dialog',{name:title,exact:true})).toBeVisible();await page.getByRole('button',{name:'关闭',exact:true}).click();}
