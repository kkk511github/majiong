...
当前 App 配置连接 `https://212.189.31.194/mahjong`（SSH 别名 `212-majiong`）。本次将0.7.2/build38部署到此服务器；以下 SafeLink-212 地址和0.4.0记录仅为历史资料。部署前保留一致数据库备份、验证副本迁移及数据行数，保留原0.7.0-build36镜像用于回退。
