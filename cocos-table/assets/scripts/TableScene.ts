...
export class TableScene extends Component {
 ...
 private hudBadge(parent:Node,name:string,x:number,y:number,w:number,h:number,fill='#063d3fb8',border='#d8c27a66',r=4){
  const n=this.make(name,x,y,w,h,parent);
  // Keep the reserved bounds, but paint nothing behind score/flower labels.
  if(this.state?.tableStyle==='reference-3d'&&name.startsWith('player-flower-count-box-'))return n;
  const g=n.addComponent(Graphics);g.fillColor=new Color(fill);g.roundRect(-w/2,-h/2,w,h,r);g.fill();g.strokeColor=new Color(border);g.lineWidth=1;g.stroke();return n;
 }
 ...
 private button(parent:Node,label:string,x:number,y:number,w:number,h:number,command:TableSceneCommand,gold=false){
  ...
  const colors=modern?(pass?['#102a38','#153241','#193b4a','#204453','#244b5a']:hu?['#124259','#1a526b','#23647d','#2f7890','#448da1']:['#0b3044','#113c50','#18495e','#20586d','#2b667d']):gold?(pass?['#0b3b3d','#0e4444','#124c4b','#155352','#185a58']:['#14504c','#1a5b55','#20665d','#267165','#2b7a6d']):['#104c4c','#155957','#1a6562','#20706b','#267b74'];
  for(let i=0;i<5;i++){g.fillColor=new Color(colors[i]);g.roundRect(-w/2+i,-h/2+i,w-2*i,h-2*i,gold?10:8);g.fill();}
  g.strokeColor=new Color(modern?(pass?'#7897a480':'#a5d4e2'):gold?(pass?'#83957788':'#c4b783'):GOLD);g.lineWidth=1.3;g.stroke();
  if(modern){g.clear();const radius=Math.min(w,h)/2-1;g.fillColor=new Color(pass?'#46667a':hu?'#398ba1':'#286073');g.circle(0,0,radius);g.fill();g.strokeColor=new Color('#b7d9e3');g.lineWidth=1;g.stroke();g.fillColor=new Color(pass?'#23475a':hu?'#d7f1ef':'#eaf3f1');g.circle(0,0,radius-4);g.fill();}
  const t=this.make(label,640,295,w-8,h-5,n),l=t.addComponent(Label);l.string=label;l.fontSize=gold?(label.length>1?25:35):20;l.color=new Color(modern?(pass?'#d9e7ed':hu?'#923d36':'#173e4c'):gold?(pass?'#b9c4a9':'#e6d5a3'):INK);l.isBold=true;l.lineHeight=44;l.horizontalAlign=Label.HorizontalAlign.CENTER;l.verticalAlign=Label.VerticalAlign.CENTER;
 ...
 private toolButton(parent:Node,kind:'trustee'|'settings',x:number,y:number,command:TableSceneCommand){
  const modern=this.state?.tableStyle==='reference-3d';
  const n=modern?this.make(`table-tool-${kind}`,x,y,64,44,parent):this.image('table-tool-jade-v2',x,y,44,44,parent);n.name=`table-tool-${kind}`;
  if(modern){const bg=n.addComponent(Graphics);bg.fillColor=new Color('#123e52');bg.roundRect(-32,-17,64,34,8);bg.fill();bg.strokeColor=new Color('#91c5d3');bg.lineWidth=1;bg.stroke();}
  const icon=this.make('tool-icon',modern?622:640,modern?295:288,20,18,n),g=icon.addComponent(Graphics);
  g.strokeColor=new Color(modern?'#edf8fb':'#efdfb9');g.fillColor=new Color(modern?'#edf8fb':'#efdfb9');g.lineWidth=1.4;
  if(kind==='trustee'){
 ...
 private clearMotion(){
  for(const id of Array.from(this.tileFlights.keys()))this.stopFlight(id);
  for(const node of Array.from(this.motionEffects)){Tween.stopAllByTarget(node);const opacity=node.getComponent(UIOpacity);if(opacity)Tween.stopAllByTarget(opacity);if(node.isValid)node.destroy();}
 ...
 private startFlight(node:Node,from:VisualTile,t:SceneTile,ms:number,arc:boolean,arrival:boolean,insertion=false){
  this.stopFlight(t.id);
  ...
  const animation=tween(progress).to(ms/1000*scale,{t:1},{easing:insertion?'linear':'quadOut',onUpdate:paint}).call(()=>{
 ...
 private draw(){
  ...
  this.drawEffect(s);this.positionDraggedTile();this.effectsRoot.setSiblingIndex(this.root.children.length-1);
  this.view3D?.render(s,tiles as Table3DTile[]);
 ...
 private drawMarks(tiles:SceneTile[]){
  ...
  const claims=this.state?.tableStyle==='reference-3d'?[]:tiles.filter(t=>t.claimTarget);
 ...
 private refreshHud(s:TableSceneState){
  const key=JSON.stringify([s.tableStyle,s.me,s.turn,s.dealer,s.presentation,s.phase,s.code,s.round,s.rounds,s.rulesName,
 ...
 private drawCompass(s:TableSceneState){
  if(!this.compassRoot){
   ...
   const compass=this.make('compass',640,s.tableStyle==='reference-3d'?247:257,122,92,this.compassRoot);
  ...
  this.compassRoot.getChildByName('compass')?.setPosition(0,295-(s.tableStyle==='reference-3d'?247:257),0);
 ...
 private drawHud(s:TableSceneState){
  ...
  const modern=s.tableStyle==='reference-3d';
  this.text(h,s.presentation==='replay'?'牌局回放':s.practice?'单人练习':`好友桌 ${s.code}`,modern?95:80,modern?78:24,150,modern?20:28,modern?14:18,INK).name='table-room-title';
  if(!modern)this.text(h,`${s.rulesName ? s.rulesName+' · ' : ''}${s.rounds ? s.round+' / '+s.rounds+' 把' : '第 '+s.round+' 把'}`,80,51,156,22,16,'#b9d1bf').name='table-room-rules';
  const multiplierX=modern?Math.max(95,(s.safeArea?.left??0)+82):80;
  if(s.roundMultiplier!==undefined)this.text(h,`${s.roundMultiplier>1?'比下胡':'本把'} × ${s.roundMultiplier}`,multiplierX,modern?270:77,148,24,18,GOLD).name='table-round-multiplier';
  ...
  for(const p of s.players){
   const o=sceneOffset(p.seat,s.me),status=scenePlayerStatus(s,p),info=layoutPlayerHud(o,s.safeArea,s.tableStyle);
   ...
   const marker=(x:number,y:number,w:number)=>{
    ...
    const badge=this.plate(h,x,y,w,23,status.tone==='offline'?'#633c2cf5':modern?'#123e52f5':'#123c33f5',statusColor,5);badge.name=`player-status-${p.seat}`;
  ...
  if(prompt&&!s.externalControls){
   ...
   const card=this.plate(h,modern?86:112,modern?316:360,modern?140:180,modern?64:86,modern?'#10384bcc':'#07494af5',modern?'#81b2c266':'#d0bd79',10);card.name='claim-prompt';
   this.text(h,`${prompt.source} · ${prompt.kind==='robKong'?'补杠':'打出'}`,modern?86:99,modern?296:330,modern?132:144,20,modern?13:15,modern?'#b3cdd8':INK);
   this.image('own-'+tileKind(prompt.tile),modern?55:59,modern?326:369,35,52,h).name='claim-prompt-tile';
   this.text(h,prompt.name,modern?116:136,modern?317:357,modern?70:100,25,22,modern?'#eef8fb':GOLD);
   this.text(h,'可'+prompt.labels.join(' / '),modern?116:136,modern?340:384,modern?74:106,modern?18:24,modern?14:17,modern?'#b4d2df':INK);
  }
  ...
  if(!this.trusteeButton){
   const n=this.toolButton(this.root,'trustee',s.tableStyle==='reference-3d'?1230:1235,35,{type:'trustee',enabled:true});
  ...
  this.trusteeButton.setPosition((modern?Math.min(1230,1280-(s.safeArea?.right??0)-50):1235)-640,295-35,0);
  if(modern){const bg=this.trusteeButton.getComponent(Graphics);if(bg){bg.clear();bg.fillColor=new Color(me.trustee?'#1b647b':'#123e52');bg.roundRect(-32,-17,64,34,8);bg.fill();bg.strokeColor=new Color(me.trustee?'#a7edf5':'#91c5d3');bg.lineWidth=1;bg.stroke();}}
 ...
 private actionPosition(s:TableSceneState,seat:number){
  ...
  for(let index=0;index<4;index++){const p=layoutPlayerHud(index,s.safeArea,s.tableStyle);obstacles.push({x:p.x,y:p.plateY,w:p.w,h:p.h});}
  obstacles.push({x:640,y:s.tableStyle==='reference-3d'?247:257,w:124,h:96},{x:531,y:250,w:72,h:66},{x:772,y:250,w:72,h:66},
   // The compact opposite flower badge sits below its score, not on a rail.
   {x:layoutPlayerHud(2,s.safeArea,s.tableStyle).x,y:89+layoutPlayerHud(2,s.safeArea,s.tableStyle).dy,w:76,h:45});
 ...
 private actionWord(s:TableSceneState,seat:number,value:string){
  ...
  if(o===0&&s.tableStyle!=='reference-3d'){
  ...
  for(const active of Array.from(this.motionEffects))if(active.name==='motion-action-'+seat){
  ...
  const g=node.addComponent(Graphics);g.fillColor=new Color('#123d50d9');g.roundRect(-width/2,-25,width,50,15);g.fill();
  g.strokeColor=new Color('#a3dcea99');g.lineWidth=1;g.roundRect(-width/2,-25,width,50,15);g.stroke();
  // Text only: the former ornament below the glyphs looked like a stray dot.
  this.text(node,value,640,295,width-8,48,value==='补花'?32:38,'#ecfaff').name='table-action-value';
  const opacity=node.addComponent(UIOpacity),scale=this.animationScale();opacity.opacity=0;
  if(!window.matchMedia('(prefers-reduced-motion: reduce)').matches){
   node.setScale(.88,.88,1);tween(node).to(.13*scale,{scale:new Vec3(1.04,1.04,1)},{easing:'sineOut'}).to(.10*scale,{scale:new Vec3(1,1,1)}).start();
   const halo=this.make('action-confirm-halo',640,295,width+16,64,node),ring=halo.addComponent(Graphics);ring.strokeColor=new Color('#a8e7f0');ring.lineWidth=1.6;ring.roundRect(-width/2-5,-30,width+10,60,19);ring.stroke();const fade=halo.addComponent(UIOpacity);fade.opacity=160;
   tween(halo).to(.46*scale,{scale:new Vec3(1.12,1.12,1)},{easing:'sineOut'}).start();tween(fade).to(.46*scale,{opacity:0},{easing:'sineOut'}).start();
  }
