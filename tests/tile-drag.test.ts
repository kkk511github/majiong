...
describe('selected hand tile drag', () => {
  it('requires a selected tile and a deliberate upward release', () => {
    ...
    expect(shouldDiscardDraggedTile(state, origin, 8, 60)).toBe(true);
    for (const [x, y] of [[0, 0], [0, 51], [0, -90], [90, 60], [Infinity, 70], [0, NaN]])
      expect(shouldDiscardDraggedTile(state, origin, x, y)).toBe(false);
    const unselected = beginTileDrag(state, 4)!;
    expect(shouldDiscardDraggedTile(state, unselected, 0, 90)).toBe(false);
  });
  ...
  it('retains physical tile identity across countdown pushes and hand sorting', () => {
    ...
    expect(shouldDiscardDraggedTile(updated, origin, 0, 70)).toBe(true);
    updated.players[0].hand = [12, 4];
    expect(shouldDiscardDraggedTile(updated, origin, 0, 70)).toBe(false);
  });
  ...
  it('allows selecting between turns without turning that touch into a discard', () => {
    ...
    expect(shouldDiscardDraggedTile(state, origin, 0, 90)).toBe(false);
    expect(shouldDiscardDraggedTile(table(), origin, 0, 90)).toBe(false);
  ...
  it('cancels across action, round, room, perspective or selection changes', () => {
    ...
    for (const change of changes) {
      ...
      expect(shouldDiscardDraggedTile(updated, origin, 0, 90)).toBe(false);
    }
  });
