...
export function ruleSections(
  ...
  if (table) {
    ...
    sections[6] = [
      "约局与托管",
      "约局大厅由管理员或已获授权的牌友开桌，四人准备后开局。管理员可配置三名机器人参与的服务器体验桌，使用正式桌规则和计时。托管会自动选牌出牌，并处理碰、杠、胡，可随时取消；打开设置或进入后台不暂停服务器牌局。准备方式、出牌时间和续桌以每桌设置为准，进行中的牌桌仅管理员可以解散。",
    ];
  }
  if (isNanjingV2(rules)) {
    ...
    sections[4] = [
      "杠与承包",
      `直杠由本次出牌者付 ${v.openKongPoints} 分；先碰后自己摸到第四张补杠，由原供碰者付 ${v.openKongPoints} 分；暗杠由其他三家各付 ${v.concealedKongPoints} 分；花杠每家 ${v.flowerKongPoints} 分。${rules.doubleSidePayments ? "以上即时收分再乘本把倍率。" : "以上即时收分不乘本把倍率。"}${b ? "暗杠、直杠、补杠和花杠的即时分不乘花砸2；胡牌时仍另算杠的软花。" : ""}暗杠展示三张牌背和一张牌面。抢杠成功不收本次杠分；普通顺子单钓不触发三嘴外包；抢杠胡由补杠者付三份。连续补杠开花保留最初供牌者；杠后又补花胡牌按小杠开花计分，仍保留点杠者付款责任。三嘴须前三嘴均来自同一家（暗杠计一嘴，至少一嘴由该家供牌），对对胡或全球独钓时承包；先碰另一家、后连续碰三嘴不成立。三清须前三嘴为同花色明碰，同花色点炮按清一色外包，自摸不触发三清外包。${isGarden(rules) ? "三口承包、清一色外包及符合架牌条件的全球独钓外包均桌外单独记分：普通50分，比下胡100分，不按胡牌分乘三、不扣桌内余额，不触发两家干或保米；外包计入最终输赢。" : "承包按胡牌分的三份在桌内结算。"}快照：三嘴后第四组碰牌，或三清后第四组同色碰牌，可忽略剩余两张，出现胡按钮后可主动胡牌；顺子不能触发快照。`,
    ];
    sections[5] = [
      "结算与过水",
      `普通点炮付一份，自摸其他三家各付一份。多人同时胡牌统一结算；${rules.twoBankrupt ? "桌内余额归零的玩家不能被点胡或抢杠胡；付款不足按各家应得比例分配，两家归零结束本桌。" : "敞开头允许记负分，按约定局数结算。"}${rules.protectWinner ? "胡牌或杠牌即时收分导致两家归零时，本次胡牌者或杠牌者不足 100 分的，优先从其他余额最多玩家的可用分补足；多人胡牌不相互扣回保米。" : ""}放弃胡或碰后，须自己下一次出牌才解除对应过水限制。`,
    ];
    sections.push([
      "天胡、地胡与比下胡",
      `庄家补完起手花牌后直接成牌为天胡，${b ? "其他三家每家固定付400分，余额不足付剩余全部；不叠加其他牌型、花分或本把倍率。两家归零则结束本桌。" : "其他三家可用分归零。"}非庄家起手听牌${b ? "，打出第一张牌并保留起手听口时自动地胡报听，" : "且"}此后不碰、不换听，胡牌加 ${v.earthly}；暗杠、明杠、补杠后听口只能不变或减少。只有庄家胡牌或流局才连庄，其他情况一律由下家坐庄。庄家胡、大胡、海底捞月、流局、包牌、花杠、弃牌奖励或罚分、一炮多响触发下一把比下胡；是否翻倍不决定谁坐庄。${rules.successorDouble ? "只有下一位庄家在本把胡牌且下一把接庄时，才触发接庄比；其他闲家普通胡牌后的换庄不因此翻倍。" : ""}${rules.biXiaHu === "off" ? "本桌关闭比下胡。" : rules.biXiaHu === "cumulative" ? "连续触发时下把倍率在当前倍率上 ×2。" : "触发后下一把 ×2，连续触发仍为 ×2，同一把多个条件不重复相乘。"}${rules.doubleSidePayments ? (b ? "本把倍率用于胡牌、杠分、四张同牌罚分及两项风牌收付。" : "本把倍率同时用于胡牌、杠分、罚分。") : "本把倍率只用于胡牌分。"}`,
    ]);
    sections.push(["架牌", "已有三组明碰，再碰第四组，随后打出的那张牌标黄并建立架牌；暗杠或直杠补牌后剩一张不建立架牌。同花色前后两张（含架牌本身）被实际胡牌时才判外包，架风牌仍按四风范围；黄色只提示风险，不限制出牌。换听立即同步取消黄色和外包，换回也不恢复；摸切或换同点数的另一张牌不取消。非法胡牌由系统直接禁止，不收诈胡罚分。"]);
    sections.push([
      "风牌奖励与罚分",
      b
        ? `${rules.fourWinds ? "自己开局前四次出牌须连续打出东南西北各一张，顺序不限，其他每家付5分；夹入其他牌即失效，后面补齐不算。" : "本桌关闭四连风奖励。"}${rules.discardPenalties ? `首轮庄家出风牌，其他三家按座序各自第一张跟同一种风牌，庄家向其他每家付5分；碰杠会打断跟牌。单人累计打齐同种四张，向其他每家付5分。` : "本桌关闭弃牌罚分。"}以上收付不乘花砸2。${rules.doubleSidePayments ? "再乘本把倍率：普通局每家5分，比下胡2倍局每家10分。" : "本桌关闭即时收付跟倍，每家仍为5分。"}`
