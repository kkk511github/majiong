...
当前 App 配置连接 `https://212.189.31.194/mahjong`（SSH 别名 `212-majiong`）。0.7.3/build39 已部署，容器健康，完整结果见 [发布说明](RELEASE-0.7.3.md)。已保存数据库备份、验证副本启动和生产数据行数，原0.7.0-build36镜像保留用于回退。以下 SafeLink-212 地址和0.4.0记录仅为历史资料。
